...
while [[ $PAGES_SCRAPED -lt $MAX_PAGES && -n "$PAGE_URL" ]]; do
    ...
    $FETCHER "$STATION_URLS_CSV" "station" | while read -r DETAIL; do
        ...
        while read -r STREAM_OBJ; do
            ...
            PROBE_OUT=$(timeout 25 ffprobe -user_agent "$UA" -headers "Referer: https://mytuner-radio.com/" -v error -select_streams a:0 -show_entries stream=codec_name -of default=noprint_wrappers=1:nokey=1 "$S_URL" 2>&1)
